...
public class GreaterThanOrEqualsToPredicate extends AbstractPredicate {
	...
	public <T> Predicate toPredicate(Class<T> clazz, Root<T> root, CriteriaQuery<?> cq, CriteriaBuilder cb, Map<String, AttributePath> paths) {
		...
		if (dataFieldType == LocalDate.class) return cb.greaterThanOrEqualTo(path.as(LocalDate.class), LocalDate.parse((String) this.getValue(), DateTimeFormatter.ISO_DATE));
		if (dataFieldType == LocalDateTime.class) return cb.greaterThanOrEqualTo(path.as(LocalDateTime.class), LocalDateTime.parse((String) this.getValue(), DateTimeFormatter.ISO_DATE_TIME));
